    ...
    while importCode[codeLineID] != "{end}":
      ...
      if ''.join(codeLine[charCounter:charCounter + 3]) == "say":
          ...
          if codeLine[charCounter] == '"':
            ...
            try:
              ...
              while codeLine[charCounter] != ';':
                cont.append(codeLine[charCounter])
                charCounter += 1
              #check does it exist and is it variable or function
        ...
        if codeLine[charCounter] == "(":
        	...
        	codeLineID += 1
      elif ''.join(codeLine[charCounter:charCounter + 2]) == "if":
        #get attributes
        charCounter += 2
        if codeLine[charCounter] == "(":
          charCounter += 1
          a1 = []
          #get first attribute
          while ''.join(codeLine[charCounter:charCounter + 2]) != " =" and ''.join(codeLine[charCounter:charCounter + 2]) != " >" and ''.join(codeLine[charCounter:charCounter + 2]) != " <" and ''.join(codeLine[charCounter:charCounter + 2]) != " !":
            a1.append(codeLine[charCounter])
            charCounter += 1
          charCounter += 1
          a2 = ''.join(codeLine[charCounter:charCounter + 2]).rstrip()
          #necessary thing, 'cuz for ex. "==" has to symbols and "<" has only one
          if a2 == ">" or a2 == "<":
            charCounter -= 1
          charCounter += 3
          a3 = []
          ...
          while codeLine[charCounter] != ")" and codeLine[charCounter] != " ":
            a3.append(codeLine[charCounter])
            charCounter += 1
          #tests if()
          if a2 == "==":
            if ''.join(a1) == ''.join(a3):
